import React from "react";

interface IconProps extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
}

const Autorenew: React.FC<IconProps> = ({ size = 24, ...props }) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width={size}
    height={size}
    viewBox="0 0 24 24"
    fill="none"
    {...props}
  >
    <path fillRule="evenodd" clipRule="evenodd" d="M12 9V6C8.69 6 6 8.69 6 12C6 13.01 6.25 13.97 6.7 14.8L5.24 16.26C4.46 15.03 4 13.57 4 12C4 7.58 7.58 4 12 4V1L16 5L12 9ZM17.3 9.20219L18.76 7.74219C19.54 8.97219 20 10.4322 20 12.0022C20 16.4222 16.42 20.0022 12 20.0022V23.0022L8 19.0022L12 15.0022V18.0022C15.31 18.0022 18 15.3122 18 12.0022C18 10.9922 17.74 10.0422 17.3 9.20219Z" fill="currentColor"/>
  </svg>
);

Autorenew.displayName = "Autorenew";
export default Autorenew;
