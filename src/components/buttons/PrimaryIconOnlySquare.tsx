/**
 * Button / Primary / Icon Only / Square  ·  PrimaryIconOnlySquare
 *
 * Figma source files (one per state):
 *   Default  → Button-6-7878  svg-56afinbn51   bg #14803c  icon white
 *   Hover    → Button-6-7890  svg-0a6m2q54ak   bg #19a04b  icon white
 *   Focus    → Button-6-7898  svg-47t1tuj86b   bg #14803c  icon white  + focus ring
 *   Pressed  → Button-6-7906  svg-klb514dy1e   bg #0c612c  icon #F3F4F6
 *   Disabled → Button-6-7882  svg-h6slm6ue6d   bg #dfe2e7  icon #C2C7D0
 *   Loading  → Button-6-7914  (no icon path)   bg #0c612c  spinner --annotation-mid-pink
 *
 * Structure (exact Figma):
 *   Button wrapper — p-[12px] rounded-[8px] overflow-clip
 ...
 *
 * Relationship to PrimaryIconOnlyCircle:
 *   Identical in every dimension (padding, icon, tokens, spinner).
 *   The ONLY difference is borderRadius — 8px (square) vs 100px (circle).
 *
...
// ─── SVG path — identical across all active-state Figma files ─────────────────
// viewBox "0 0 14 14"
const PLUS_PATH = "M14 8H8V14H6V8H0V6H6V0H8V6H14V8Z";

// ─── Types ────────────────────────────────────────────────────────────────────

export type IconOnlySquareState =
  | "default"
  | "hover"
  | "focus"
  | "pressed"
...
> = {
  default: {
    bg:        "var(--fill-default-enabled-primary)",   // #14803c green-60
    iconColor: "var(--icon-default-enabled-primary)",   // white
    boxShadow: "none",
    cursor:    "pointer",
  },
  hover: {
    bg:        "var(--fill-default-hover-primary)",      // #19a04b green-50
    iconColor: "var(--icon-default-hover-primary)",      // white
    boxShadow: "none",
    cursor:    "pointer",
  },
  focus: {
    bg:        "var(--fill-default-focus-primary)",      // #14803c green-60
    iconColor: "var(--icon-default-focus-primary)",      // white
    boxShadow: "var(--focus-border)",                    // 0 0 0 3px rgba(0,146,228,0.5)
    cursor:    "pointer",
  },
  pressed: {
    bg:        "var(--fill-default-pressed-primary)",    // #0c612c green-70
...
function PlusIcon({ fill }: { fill: string }) {
  return (
    // Outer — content-stretch flex items-center relative shrink-0 size-[24px]
    <div
      style={{
        display:    "flex",
        alignItems: "center",
    ...
    >
      {/* Middle — aspect-[28/28] h-full overflow-clip relative shrink-0 */}
      <div
        style={{
          aspectRatio: "28 / 28",
          height:      "100%",
      ...
      >
        ...
        >
          {/* SVG — absolute block size-full preserveAspectRatio="none" viewBox="0 0 14 14" */}
          <svg
            style={{
              position: "absolute",
              display:  "block",
...
export interface IconOnlySquareAtomProps {
  /** Pin the button to a specific state (for showcase/frozen previews) */
  forcedState?: IconOnlySquareState;
  /** Accessible label — rendered as aria-label */
  ariaLabel?: string;
  onClick?: () => void;
}

export function IconOnlySquareAtom({
  forcedState,
  ariaLabel = "Action",
  onClick,
}: IconOnlySquareAtomProps) {
  const [live, setLive] = useState<IconOnlySquareState>("default");
  ...
  const set = (next: IconOnlySquareState) => () => {
    if (!forcedState) setLive(next);
  };

  return (
    // Figma: bg-[COLOR] content-stretch flex items-center justify-center
    //        overflow-clip p-[12px] relative rounded-[8px] size-full
    // Note: box-shadow (focus) is on the element itself — not clipped by overflow.
    <button
      disabled={isDisabled}
      aria-busy={isLoading}
      ...
      style={{
        ...
        overflow:       "clip",
        padding:        "12px",
        borderRadius:   "8px",           // ← square, not circle
        position:       "relative",
        flexShrink:     0,
        // Colour tokens
        background: s.bg,
    ...
    >
      {isLoading ? (
        ...
        // → shared Spinner at 24 px to match the icon footprint
        <Spinner
          size={24}
...
const STATES: {
...
}[] = [
  {
    id:          "default",
    name:        "Default",
    figmaFile:   "Button-6-7878 · svg-56afinbn51",
  ...
  },
  {
    id:          "hover",
    name:        "Hover",
    figmaFile:   "Button-6-7890 · svg-0a6m2q54ak",
  ...
  },
  {
    id:           "focus",
    name:         "Focus",
    figmaFile:    "Button-6-7898 · svg-47t1tuj86b",
  ...
  },
  {
    id:          "pressed",
    name:        "Pressed",
    figmaFile:   "Button-6-7906 · svg-klb514dy1e",
  ...
  },
  {
    id:          "disabled",
    name:        "Disabled",
    figmaFile:   "Button-6-7882 · svg-h6slm6ue6d",
  ...
  },
  {
    id:          "loading",
    name:        "Loading",
    figmaFile:   "Button-6-7914 · (no path — spinner only)",
    bgToken:     "--fill-default-pressed-primary",
    ...
    >
      ...
      >
        ...
        <header style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          ...
          >
            Cropwise Design System V2 · buttons / Primary / Icon Only / Square
          </p>
          ...
          >
            A square icon-only button with no text label and an{" "}
            <code style={{ fontFamily: "monospace", fontSize: "13px" }}>8px</code>{" "}
            corner radius. Six interaction states — identical tokens to the Circle
        ...
        >
          ...
          >
            ...
            />
            <IconOnlySquareAtom forcedState="hover"    />
            <IconOnlySquareAtom forcedState="focus"    />
            <IconOnlySquareAtom forcedState="pressed"  />
            <IconOnlySquareAtom forcedState="disabled" />
          </div>
        </section>
          ...
          {STATES.map((row, idx) => (
            ...
            >
              ...
              <div style={{ display: "flex", justifyContent: "center" }}>
                <IconOnlySquareAtom forcedState={row.id} />
              </div>
            </div>
          ))}
        </section>
        ...
        >
          ...
          >
            {[
              { prop: "Shape",       value: "Square — rounded-[8px]",       figma: "rounded-[8px]" },
              { prop: "Padding",     value: "12 px uniform",                figma: "p-[12px]" },
