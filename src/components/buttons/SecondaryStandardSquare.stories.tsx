...
import { SecondarySquareAtom } from "./SecondaryStandardSquare";

const meta: Meta<typeof SecondarySquareAtom> = {
  title: "Design System/Buttons/Secondary/Standard Square",
  component: SecondarySquareAtom,
  tags: ["autodocs"],
  argTypes: {
    forcedState: {
      control: "select",
...
export default meta;
type Story = StoryObj<typeof SecondarySquareAtom>;
  ...
  render: () => (
    <div style={{ display: "flex", gap: 16, flexWrap: "wrap", alignItems: "center" }}>
      {(["default", "hover", "focus", "pressed", "disabled", "loading"] as const).map((s) => (
        <SecondarySquareAtom key={s} forcedState={s} label={s.charAt(0).toUpperCase() + s.slice(1)} />
      ))}
    </div>
  ),
};
