...
import { TertiaryStandardSquareAtom } from "./TertiaryStandardSquare";

const meta: Meta<typeof TertiaryStandardSquareAtom> = {
  title: "Design System/Buttons/Tertiary/Standard Square",
  component: TertiaryStandardSquareAtom,
  tags: ["autodocs"],
  argTypes: {
    forcedState: {
      control: "select",
...
export default meta;
type Story = StoryObj<typeof TertiaryStandardSquareAtom>;
  ...
  render: () => (
    <div style={{ display: "flex", gap: 16, flexWrap: "wrap", alignItems: "center" }}>
      {(["default", "hover", "focus", "pressed", "disabled", "loading"] as const).map((s) => (
        <TertiaryStandardSquareAtom key={s} forcedState={s} label={s.charAt(0).toUpperCase() + s.slice(1)} />
      ))}
    </div>
  ),
};
