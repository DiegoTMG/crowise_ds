...
// Figma: Android-status-bar / Dark-mode=false | Dark-mode=true
// Time on the left; Cellular + WiFi + Battery on the right.

export type StatusBarMode = "light" | "dark";

export interface AndroidStatusBarProps {
  mode?: StatusBarMode;
  time?: string;
  className?: string;
}
...
export function AndroidStatusBar({
  ...
  // Figma: light bg = #FFFFFF (Neutral-00), dark bg = #0F0F0F (colors/backgrounds/dark)
  const bg = isDark ? "#0f0f0f" : "var(--mono-white-100)";
  // Figma: light fg = #170E2B (wireframes/text-legibility/high-emphasis), dark fg = #FFFFFF @0.9
  const fg = isDark ? "var(--mono-white-100)" : "#170E2B";
